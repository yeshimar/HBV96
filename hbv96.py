...
class HBV96(HydroModel):
	...
	def calibrate(self):
		...
		obj_fun=self.config['obj_fun']
		x_0 = self.config['init_guess']
		
		def _cal_fun(par_to_optimize):
			...
			if self.config['verbose']:
				print('{0}: {1}'.format(self.config['fun_nam'], perf))

			if self.config['minimise']:
				return obj_fun(_q_rec[self.config['wu']:], _q_sim[self.config['wu']:])
			else:
				return -obj_fun(_q_rec[self.config['wu']:], _q_sim[self.config['wu']:])
