...
class HBV96(HydroModel):
	...
	def calibrate(self):
		...
		obj_fun=self._config['obj_fun']
		x_0 = self._config['init_guess']
		...
		def _cal_fun(self, par_to_optimize):
			...
			if self._config['verbose']:
				print('{0}: {1}'.format(self._config['fun_nam'], perf))

			if self._config['minimise']:
				return obj_fun(_q_rec[self._config['wu']:], _q_sim[self._config['wu']:])
			else:
				return -obj_fun(_q_rec[self._config['wu']:], _q_sim[self._config['wu']:])
